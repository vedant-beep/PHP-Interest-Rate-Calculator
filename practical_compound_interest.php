...
foreach($payments as $k=>$val){// skipping the first payment
...
function getAggregateInt($currpaydate, $interestRates,$dueLeftToPay){
    ...
    if(!empty($arr)){
        ...
        for($i = 1; $i<count($arr); $i++){
            ...
            $total_si += ci($dueLeftToPay,$interestRates[$arr[$i-1]],$days);
        }
        $l_days = getTimeDiff($currpaydate,$arr[$i-1]);
        $total_si += ci($dueLeftToPay,$interestRates[$arr[$i-1]],$l_days);
    }
    return $total_si;
}
